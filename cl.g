...
program: PROGRAM^ innerprogram ENDPROGRAM! INPUTEND!;

innerprogram: dec_vars l_dec_blocs l_instrs;
...
dec_bloc: (PROCEDURE^ inbloc innerprogram  ENDPROCEDURE! |
           FUNCTION^  inblocreturn innerprogram RETURN! cond  ENDFUNCTION!);

inblocreturn: IDENT^ inblocb RETURN! constr_type;

inbloc: IDENT^ inblocb;

inblocb: OPENPAR! (dec_params ((COMA! dec_params)+ | ) | ) CLOSEPAR! <<#0=createASTlist(_sibling);>>;

dec_params: (VAL^| REF^) IDENT (constr_type);

constr_type: INT | BOOL | STRUCT^ (field)* ENDSTRUCT!
		|ARRAY^ OPENBRK! INTCONST CLOSEBRK! OF! constr_type ;
...
instruction:
        identif ( (ASIG^ cond) | (OPENPAR^ lista_params CLOSEPAR!))
      | WHILE^ cond DO! l_instrs ENDWHILE!
      |IF^ cond THEN! l_instrs (ELSE! l_instrs |) ENDIF!
      |syscall
;

cond: expcomp ((OR^ | AND^) expcomp)*; 

expcomp : expsuma ((LESSER^ | EQUAL^ | GREATER^) expsuma)*;

expsuma:  expmul ((PLUS^|MINUS^) expmul)*;

expmul: expnot ((MUL^|DIV^)expnot)*;

expnot: (MINUS^|NOT^) expnot | (OPENPAR! cond CLOSEPAR!) | expsimple ;

expsimple:
       identif((OPENPAR^ lista_params CLOSEPAR!)(DOT^ IDENT | OPENBRK^ cond CLOSEBRK!)*|)
      | INTCONST | STRING | bool_val
      ;

identif: IDENT (DOT^ IDENT | OPENBRK^ cond CLOSEBRK!)*;

bool_val:  BTRUE | BFALSE;

lista_params: (params |) <<#0=createASTlist(_sibling);>>;

params: cond (COMA! params |);

syscall: (READ^ | WRITE^ | WRITELN^)  OPENPAR! (cond) CLOSEPAR!;
